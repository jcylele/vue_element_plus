...
export class DownloadLimit extends EditableData {
    ...
    get limit_desc_list(): string[] {
        ...
        if (this.limit.single_file_size > 0) {
            desc_list.push(`single ${this.format_file_size(this.limit.single_file_size)}`)
        }

        // total file size
        if (this.limit.total_file_size > 0) {
            const progress = this.format_file_size(this.progress.total_file_size)
            const limit = this.format_file_size(this.limit.total_file_size)
