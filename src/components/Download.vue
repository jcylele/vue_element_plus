<template>
	<el-space direction="vertical" fill>
		...
		<el-space v-if="show_category" direction="horizontal" class="with-border">
			...
			<el-select v-model="actor_category" placeholder="Select" class="extra-select">
		...
		<el-space v-if="by_new" direction="horizontal" class="with-border">
			...
			<el-select v-model="start_page_type" placeholder="Select" class="extra-select">
			...
			switch (this.down_type) {
				case DownloadType.New: {
					...
					if (this.start_page_type === EStartPage.Custom) {
						...
						real_start_page = this.start_page_type.value
					}
					[ok, ret] = await downloadNewActors(this.download_limit, this.actor_category, real_start_page)
					break
				}
